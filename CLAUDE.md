...
- `revit-pyrevit-extention/` — pyRevit extension hosting the IronPython 2.7 execution provider. UI-less: `startup.py` registers the provider disabled; the C# ribbon's Python toggle enables/disables/reloads it via the registered delegates (the old `Revit MCP.tab` name-merge constraint is gone).
...
Before driving the live bridge, read `GOTCHAS.md` — operational traps (family-doc transaction patterns, manual-mode `Commit()` status, IronPython `.Name`/encoding pitfalls) and which historical traps are fixed.
...
- **Discovery**: bridge writes `%LOCALAPPDATA%\RevitMcp\instances\<pid>.json` on Bridge ON; the Python side lists/validates these (PID-reuse rejection). Both capabilities start OFF every Revit session — a human click on the two ribbon toggles (Bridge, Python) is required each session by design.
...
- **Providers**: IronPython 2.7 lives in pyRevit's persistent engine — module edits only take effect after the Python toggle is clicked ON or `reload_python_provider` runs (the registered reload delegate re-reads the module from disk; the engine caches imports). C# runs through the isolated Roslyn provider (`providers/roslyn/1/`, collectible AssemblyLoadContext); allowed compile references are the explicit list in `RoslynProviderHost.ReferenceManifest` — missing-assembly compile errors are fixed there.
...
- **Pane data files**: MCP server writes `%LOCALAPPDATA%\RevitMcp\mcp-tools.json` at startup (tool list + LLM descriptions); the Activity pane's Tools/Saved views read these files, never the pipe. Operator settings moved out of the pane into the ribbon **Settings** dialog (`SettingsDialog.cs`); `LocalSettingsStore` round-trips unknown settings.json keys so a pane/dialog write can't wipe newer or Python-side keys.
- **Activity summaries**: the handler counts added/modified/deleted element ids per request via `DocumentChanged` and logs them as `summary` (`+a ~m -d`); agents pass an optional `label` (intent phrase) with `run_python`/`run_csharp`, and `run_saved_tool` auto-labels with the tool id. Rows render label bold, facts muted.
